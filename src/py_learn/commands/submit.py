...
def handle(
    ...
    if not current_id:
        console.print("[red]当前答题目录没有承载任何练习。请先使用 pl new 或 pl resume。[/]")
        return

    exercise = exercises.get(current_id)
    if not exercise:
        console.print(f"[red]内部错误：未找到练习 {current_id}[/]")
        return

    source_path = wm.get_current_source_path(workspace_path)
    if not source_path:
        console.print(f"[red]无法定位原始练习目录。请重新使用 pl new {current_id}。[/]")
        return
    ...
    try:
        ...
        console.print(f"[red]加载答题文件失败: {e}[/]")
        return

    try:
        _, submit_cases = load_cases(cases_path)
    except Exception as e:
        console.print(f"[red]加载验证文件失败: {e}[/]")
        return

    if not submit_cases:
        console.print("[yellow]该练习没有提交验证用例（SUBMIT_CASES）。[/]")
        return
